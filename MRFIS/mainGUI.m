...
if isInit                                     % Initilize
    filename='starfish.jpg';                      % Load the default image
    filepath='./testImages/'; 
else                                         % Load a new image
    [filename filepath]=uigetfile('*.jpg','Load an image');
    if filename==0
        return;
    end
end
...
%¼ÆËãÍ¼ÏñµÄ³¬ÏñËØ
% orgImageTmp = orgImage;
[L,N] = superpixels(orgImage,600);
Label=L;              % conver the segImage into the labelled image to region merging next.
